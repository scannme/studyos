...
{
    if (NULL == mbsp || 0 == mbsp->mb_e820exsz || NULL == mbsp->mb_e820expadr|| 0==mbsp->mb_e820nr) {
        *retpmrsz = 0;
        *retpmrvadr  = NULL;
        return;
    }
...
void init_phymmarge()
{
    ...
    u64_t pmrgesz = 0;
    //根据machbstart_t 机器信息结构计算获得phymmarge_t结构的开始和大小
    ret_phymmarge_adrandsz(mbsp, &pmarge_adr, &pmrgesz);

    if (NULL == pmarge_adr || 0 == pmrgesz) {
        system_error("init phymmarge is null or pmrgesz is 0\n");
        return;
    }

    u64_t tmppmrphyadr = mbsp->mb_nextwtpadr;
    if ((adr_t)tmppmrphyadr != ((adr_t)pmarge_adr)) {
        system_error("init phymmarge tmppmrphyadr != pmarge adr2phyadr");
        return;
    }
    ...
    if (ipmgnr == 0) {
        system_error("init phymmarge ret 0\n");
        return;
    }

    if ((ipmgnr * sizeof(phymmarge_t)) != pmrgesz) {
        system_error("init phymarge size is not equal pmrgesz");
        return;
    }
