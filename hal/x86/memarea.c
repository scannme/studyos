...
void memdivmer_t_init(memdivmer_t *initp)
{
    ...
    initp->dm_divnr = 0;
    initp->dm_mernr =0;
    //循环初始化memdivmer_t结构体中dm_mdmlielst数组中每个bafhlst_t结构体
    for (uint_t li = 0; li < MDIVMER_ARR_BMAX; li++) {
        bafhlst_t_init(&initp->dm_mdmlielst[li], BAFH_STUS_ONEM, li,(1UL << li));
    }
...
bool_t init_memarea_core(machbstart_t *mbsp)
{
    ...
    //将memarea_t结构的开始物理地址写入kmachbsp结构中
...
void init_memarea()
{
    ...
    if (init_memarea_core(&kmachbsp) == FALSE) {
        system_error("init_memarea_core");
    }
...
uint_t merlove_setallmarflgs_onmemarea(memarea_t *mareap, msadsc_t *mstat, uint_t msanr)
{
    ...
    case MA_TYPE_HWAD:
        ...
        break;
    case MA_TYPE_KRNL:
        muindx = MF_MARTY_KRL << 5;//用内核区标签
        mdfp = (msadflgs_t *)(&muindx);
        break;
    case MA_TYPE_PROC:
        muindx = MF_MARTY_PRC << 5;//应用标签
        mdfp = (msadflgs_t *)(&muindx);
        break;
    }
...
bool_t continumsadsc_add_bafhlst(memarea_t *mareap, bafhlst_t *bafhp, msadsc_t *fstat, msadsc_t *fend, uint_t fmnr)
{
    ...
    //把多个地址连续的msadsc_t结构开始的那个msadsc_t结构挂载到bafhlst_t结构的af_frelst中
...
bool_t continumsadsc_mareabafh_core(memarea_t *mareap, msadsc_t **rfstat, msadsc_t **rfend, uint_t *rfmnr)
{
    ...
    if ((BAFH_STUS_DIVP == bafhp->af_stus || BAFH_STUS_DIVM == bafhp->af_stus)) {
        //看地址连续的msadsc_t结构的数量是否正好是bafhp->af_oderpnr
        ...
        //如果连续地址的msadsc_t结构数量正好是bafhp->af_oderpnr则完成,否则返回再次进入
        if (tmpmnr == 0) {
            *rfmnr = tmpmnr;
            *rfend = NULL;
            return TRUE;
...
bool_t merlove_continumsadsc_mareabafh(memarea_t *mareap, msadsc_t *mstat, msadsc_t *mend, uint_t mnr)
{
    ...
    //如果mnridx > 0 并且NULL != fend就循环调用continumsadsc_mareabafh_core函数, 而mnridx和fend由这个函数控制
...
bool_t merlove_mem_core(machbstart_t *mbsp)
{
    ...
    for (uint_t mi = 0; mi < (uint_t)mbsp->mb_memznnr; mi++) {
        ...
        sretf = merlove_setflgs_onmemarea(&marea[mi], mstatp, msanr);
        if ((~0UL) == sretf) {
            return FALSE;
        }
    }
...
void init_merlove_mem()
{
    if (merlove_mem_core(&kmachbsp) == FALSE) {
        system_error("merlove mem core failed\n");
    }
    return;
}
