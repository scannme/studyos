...
{
	u64_t paf_alloc:1; //分配位j
	u64_t paf_shared:1; //共享位
	u64_t paf_swap:1; //交换位
	u64_t paf_cache:1; //缓存位
	u64_t paf_kmap:1;//映射位i
...
typedef struct s_MSADSC
{
	...
	spinlock_t md_lock; //4 保存的自旋锁
	msadflgs_t md_indxflgs;//4 内存空间地址描述标志
	phyadrflgs_t md_phyadrs;//8 物理地址和标志
	void* md_odlink;//8 相邻且相同大小msadc的指针
