...
typedef struct s_BAFHLST
{
	...
	uint_t af_oderpnr; //oder对应的页面数比如order为2 那就是1<<2 =4
	uint_t af_fobjnr; //多少个空闲得msadsc_t结构,即空闲页面
	//uint_t af_aobjnr;
	uint_t af_mobjnr; //此结构的msadsc_t结构总数,即此结构总页面
	uint_t af_alcindx; //此结构的分配计数
	uint_t af_freindx;//此结构的释放计数
	list_h_t af_frelst; //挂载此结构空闲msadsc_t结构
...
typedef struct s_MEMDIVMER
{
	...
	bafhlst_t dm_onemsalst; //单个bafhlst_t结构
}memdivmer_t;
...
typedef struct s_MEMAREA
{
	...
	uint_t ma_type;//类型
	sem_t ma_sem; //信号量
	wait_l_head_t ma_waitlst; //内存区等待队列
	uint_t ma_maxpages;//总的页表数
	uint_t ma_allocpages; //分配的页表数
	uint_t ma_freepages; //空闲的页表数j
	uint_t ma_resvpages;// 保留的页面数
	uint_t ma_horizline;//分配时的水位数
